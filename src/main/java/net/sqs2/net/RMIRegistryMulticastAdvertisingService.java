...
public class RMIRegistryMulticastAdvertisingService extends MulticastAdvertisingService {
	...
	private String createMessage(String name)throws SocketException,UnknownHostException {
		String url = RMIRegistryUtil.createURL(this.rmiPort, name);
		return url + "\t" + this.key + "\t" + this.sessionID;
	}
	
	public void startAdvertising(Remote remote, final String bindingName) throws UnknownHostException{
		try {
			if (RMIRegistryUtil.export(remote, this.rmiPort, bindingName) == -1) {
				...
				startAdvertise(
						...
						, this.interval);
				Logger.getLogger("net").info(bindingName+" -> "+message);
			}
		} catch (RemoteException ex) {
			ex.printStackTrace();
			Logger.getLogger("net").warning(bindingName+" "+ex.getMessage());
